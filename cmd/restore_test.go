...
func TestRestoreManagedAndForce(t *testing.T) {
	...
	if err := mgr.Registry().Add(arc); err != nil { t.Fatalf("seed: %v", err) }

	// Case 1: restore normally
	out, err := execRestore(t, arc.UID)
	if err != nil { t.Fatalf("restore: %v; out=%s", err, out) }
	// File should be moved back to OriginalPath
	if _, err := os.Stat(arc.OriginalPath); err != nil { t.Fatalf("restored file missing: %v", err) }

	// Re-fetch record; status should be present
	updated, err := mgr.Registry().Get(name)
	if err != nil { t.Fatalf("get updated: %v", err) }
	if updated.Status != "present" || updated.DeletedAt != nil { t.Fatalf("wrong status after restore: %+v", updated) }
	...
	if err := cmd.Execute(); err != nil { t.Fatalf("restore --force: %v; out=%s", err, buf.String()) }
}
...
func TestRestoreExternalSoftDeleted(t *testing.T) {
	...
	out, err := execRestore(t, arc.UID)
	if err != nil { t.Fatalf("restore external: %v; out=%s", err, out) }
	updated, err := mgr.Registry().Get(name)
	if err != nil { t.Fatalf("get: %v", err) }
	if updated.Status != "present" || updated.DeletedAt != nil { t.Fatalf("bad status: %+v", updated) }
}
